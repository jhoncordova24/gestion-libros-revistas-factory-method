...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $revista->setUnidades($_POST['unidades']); 

    $revista->editar();

    header("Location: gestionarRevistas.php");
    exit;
}
...
if (isset($_GET['id'])) {
    ...
    $stmt = $connection->prepare("SELECT * FROM revistas WHERE id = :id");
...
<body>
    ...
    <section class="content-editar">
        <form class="editar" method="POST" action="">
            <h1 class="edit-title">Editar Revista</h1>
            ...
            <input type="text" id="editorial" name="editorial" value="<?php echo htmlspecialchars($revista['editorial']); ?>" required>

            <label for="precio">Precio:</label>
            <input type="text" id="precio" name="precio" value="<?php echo htmlspecialchars($revista['precio']); ?>" required>

            <label for="unidades">Unidades:</label> 
            <input type="number" id="unidades" name="unidades" value="<?php echo htmlspecialchars($revista['unidades']); ?>" required>
